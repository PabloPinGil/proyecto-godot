...
public class T003Button {
    ...
    public void checkRegisterButtonIsCorrect() {
        Matcher<View> parent = withChild(withText("Crear cuenta"));
        Espresso.onView(withText("Crear cuenta")).check(matches(withClassName(containsString("Button"))));
        Espresso.onView(withText("Crear cuenta")).check(isBottomAlignedWith(parent));
        Espresso.onView(withText("Crear cuenta")).check(isLeftAlignedWith(parent));
        Espresso.onView(withText("Crear cuenta")).check(isRightAlignedWith(parent));
        Espresso.onView(withText("Crear cuenta")).check(hasDeclaredId());
    }
    ...
    public void checkRegisterActivityWorksProperly() throws ClassNotFoundException, NoSuchFieldException {
        ...
        Field button = registerCls.getDeclaredField("registerButton");
        String randomText = "Nelson said haha " + ((int)(Math.random() * 20000)) + " times";
        Espresso.onView(editTextWithHint("carole.doe")).perform(typeText(randomText));
        Espresso.closeSoftKeyboard();
        Espresso.onView(withText("Crear cuenta")).perform(click());
        Matcher<View> toast = withText("Usuario: " + randomText);
