...
@LargeTest
public class T008LoginRequestCanFail extends TestRun {
    ...
    public void checkRequestSentAndServerErrorToastShown() throws JSONException {
        String randUser = "Dev21User" + (new Date()).getTime(); // This user will not exist
        String randPass = "Dev21Pass" + (new Date()).getTime();
        Espresso.onView(withHint("marylin.doe")).perform(typeText(randUser));
