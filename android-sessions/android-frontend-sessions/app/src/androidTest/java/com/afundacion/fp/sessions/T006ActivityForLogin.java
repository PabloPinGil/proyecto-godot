...
public class T006ActivityForLogin extends TestRun {
    ...
    public void checkEditTextUsernameIsCorrect() {
        Espresso.onView(editTextWithHint("marylin.doe")).check(isCenteredOnParent());
        Espresso.onView(editTextWithHint("Contraseña")).check(isTopToBottomOf(editTextWithHint("marylin.doe")));
        Espresso.onView(editTextWithHint("marylin.doe")).check(isLeftAlignedWith(editTextWithHint("Contraseña")));
        Espresso.onView(editTextWithHint("marylin.doe")).check(isRightAlignedWith(editTextWithHint("Contraseña")));
        Espresso.onView(editTextWithHint("marylin.doe")).perform(typeText("hola, hola"));
        Espresso.onView(editTextWithHint("marylin.doe")).check(hasDeclaredId());
    }
    ...
    public void checkGoToRegisterButtonWorksForthAndBack() throws JSONException {
        ...
        String randUser = "Dev21User" + (new Date()).getTime();
        String randPass = "Dev21Pass" + (new Date()).getTime();
        Espresso.onView(withHint("carole.doe")).perform(typeText(randUser));
        Espresso.closeSoftKeyboard();
        Espresso.onView(withHint("Contraseña")).perform(typeText(randPass));
        Espresso.closeSoftKeyboard();
        Espresso.onView(withText("Crear cuenta")).perform(click());
        ...
        Matcher<View> toast = withText(containsString("Usuario registrado"));
        Espresso.onView(toast).inRoot(isOverlayWindow()).check(matches(isDisplayed()));
        Espresso.onView(withText("Crear cuenta")).check(doesNotExist());
    }
